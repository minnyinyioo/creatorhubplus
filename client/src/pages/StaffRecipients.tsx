...
export type RecipientKindFilter = "all" | "Wallet" | "Bank";
export type RecipientSortOrder = "provider" | "published" | "updated";
type RecipientSummary = { paymentMethod: string; providerLabel: string; kind: string; isActive: boolean | number; updatedAt?: Date | string | null };

export function filterAndSortRecipientProviders(providers: typeof providerOptions, recipients: readonly RecipientSummary[], query: string, kindFilter: RecipientKindFilter, sortOrder: RecipientSortOrder) {
  const normalizedQuery = query.trim().toLowerCase();
  const savedByProvider = new Map(recipients.map((recipient) => [recipient.paymentMethod, recipient]));
  return providers.filter((provider) => {
    const saved = savedByProvider.get(provider.id);
    return (kindFilter === "all" || provider.kind === kindFilter) && (!normalizedQuery || provider.label.toLowerCase().includes(normalizedQuery) || saved?.providerLabel.toLowerCase().includes(normalizedQuery));
  }).sort((a, b) => {
    const savedA = savedByProvider.get(a.id);
    const savedB = savedByProvider.get(b.id);
    if (sortOrder === "published") return Number(Boolean(savedB?.isActive)) - Number(Boolean(savedA?.isActive)) || a.label.localeCompare(b.label);
    if (sortOrder === "updated") return (new Date(savedB?.updatedAt ?? 0).getTime() - new Date(savedA?.updatedAt ?? 0).getTime()) || a.label.localeCompare(b.label);
...
export function StaffRecipientsContent() {
  const { user, loading } = useAuth();
  const [selectedProvider, setSelectedProvider] = useState<ProviderId>("kbzpay");
  const [recipientSearch, setRecipientSearch] = useState("");
  const [recipientKindFilter, setRecipientKindFilter] = useState<RecipientKindFilter>("all");
  const [recipientSortOrder, setRecipientSortOrder] = useState<RecipientSortOrder>("provider");
  ...
  const visibleProviders = useMemo(() => filterAndSortRecipientProviders(providerOptions, query.data ?? [], recipientSearch, recipientKindFilter, recipientSortOrder), [query.data, recipientSearch, recipientKindFilter, recipientSortOrder]);
  ...
  return <div className="staff-page case-ledger-page">
