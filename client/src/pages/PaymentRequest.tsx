...
export default function PaymentRequest() {
  ...
  const createRequest = trpc.paymentRequest.create.useMutation({
    onSuccess: async (result) => {
      ...
      toast(`Payment request ${result.requestCode} was submitted for review.`);
      setProof(null);
      setReference("");
      setStep("details");
    },
    onError: (error) => toast(error.message || "Unable to save this payment request."),
  });
  ...
  const submitProof = async () => {
    ...
    if (!["image/png", "image/jpeg", "image/webp", "application/pdf"].includes(proof.type)) {
      toast("Use a PNG, JPG, WEBP or PDF receipt file.");
      return;
    }
    try {
      const receiptDataUrl = await fileToDataUrl(proof);
      await createRequest.mutateAsync({
        paymentMethod: selected.id as "kbzpay" | "wavepay" | "ayapay" | "kbzbank" | "ayabank" | "bangkok" | "kasikorn",
        payerName: payerName.trim(),
        accountHint: accountHint || undefined,
        amountMmk: Number(amount),
