...
export function canBeginProtectedPayment(isAuthenticated: boolean, hasSelectedService: boolean) {
  return isAuthenticated && hasSelectedService;
}
...
export default function PaymentRequest() {
  ...
  const recipientQuery = trpc.paymentRequest.recipient.useQuery(recipientInput, { enabled: Boolean(selectedServiceKey) && !loading && isAuthenticated });
  ...
  return <main className="payment-page">
    <header className="payment-header"><Link href="/" className="payment-brand"><CreatorHubPlusLockup label="CreatorHubPlus" /></Link><Link href="/" className="payment-return"><ArrowLeft size={15} /> Return to site</Link></header>
    ...
    <section className="payment-shell">
      ...
      <div className="payment-workspace">
        ...
        {!selectedService ? <section className="payment-gate"><CircleAlert size={20} /><div><strong>Payment information is locked.</strong><p>Choose the service or product above first. The payment route, amount and receipt steps will appear here after your selection.</p></div></section> : loading ? <section className="payment-gate"><ShieldCheck size={20} /><div><strong>Checking your sign-in status.</strong><p>Payment details are protected and will appear after authentication is confirmed.</p></div></section> : !canBeginProtectedPayment(isAuthenticated, Boolean(selectedService)) ? <section className="payment-gate payment-auth-gate"><ShieldCheck size={20} /><div><strong>Sign in to continue.</strong><p>Payment applications, verified merchant details, receipt uploads and invoice records are available only to authenticated users.</p><button type="button" className="payment-primary" onClick={() => startLogin()}>Sign in securely <ArrowRight size={17} /></button></div></section> : <>
